...
- **LSP visualization components**: Added completions and semantic highlighting for `ui:sparkline:` (mini charts), `ui:rating:` (star/heart ratings), and `ui:waveform:` (audio waveforms)
...
- **LSP auto-pair compatibility**: Fixed completions inserting duplicate `}}` when editor has auto-pair enabled. Completions now omit closing brackets since most editors auto-insert them
...
- **Internal**: Refactored LSP from single 2,350-line file into modular structure with separate modules for parser, completions, semantic tokens, diagnostics, code actions, and color picker
