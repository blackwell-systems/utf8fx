            ...
            "area" => {
                ...
                if points.is_empty() {
                    ...
                    let area_path = format!(
                        ...
                        path_points.join(" "), fill_color
                    );
                    let line_path = format!(
                        "  <polyline points=\"{}\" fill=\"none\" stroke=\"#{}\" stroke-width=\"{}\"/>",
                        points.iter().map(|(x, y)| format!("{:.1},{:.1}", x, y)).collect::<Vec<_>>().join(" "),
                        stroke_col, stroke_width
                    );
                    format!("{}\n{}", area_path, line_path)
                }
            }
